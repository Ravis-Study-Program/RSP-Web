  ...
  builder.Services
         .AddCors(options =>
         {
           options.AddPolicy("CorsPolicy",
                             policyBuilder => { policyBuilder.AllowAnyHeader().AllowAnyMethod().WithOrigins("*"); });
         })
         ...
         {
           options.Authority =
             "https://dev-gu5nqsqu82rthcq2.us.auth0.com/"; // $"https://{builder.Configuration["Auth0:Domain"]}/";
...
var app = builder.Build();
{
  ...
  app.Map("/error", (HttpContext context) =>
  {
    ...
    {
      Error = new ApiError(exception?.Message ?? "Error occured")
    };
  });
}
