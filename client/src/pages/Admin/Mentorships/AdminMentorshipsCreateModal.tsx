...
export const AdminMentorshipsCreateModal = ({
  ...
  seasons,
  enrollments,
}: AdminMentorshipsCreateModalProps) => {
  const form = useForm({
    mode: 'uncontrolled',
    initialValues: {
      seasonId: '',
  ...
  const handleSubmit = async (values: {
    ...
    try {
      const requestData: AdminCreateMentorshipRequest = {
  ...
  const menteeRef = useRef<HTMLInputElement>(null);
  const [selectKey, setSelectKey] = useState(0); // Add key state
